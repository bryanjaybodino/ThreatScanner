...
namespace ThreatScanner
{
    ...
    public partial class FullScannerForm : Form
    {
        ...
        private async Task CheckSqlInjectionHints(string url)
            => await SqlInjectionHelper.ProbeUrlParamAsync(_http, url, "id", AddRow);
        ...
        private async Task ProbeForms(string url, CancellationToken ct)
        {
            ...
            int fieldsTested = await SqlInjectionHelper.ProbeFormsAsync(_http, url, AddRow, AddSep, ct);
            AddRow("Form Probe", "✅ Complete", $"Tested {fieldsTested} field(s).");
        }

        private async Task ProbeFormsXss(string url, CancellationToken ct)
        {
            AddSep("Form XSS Probe");
            AddRow("Form XSS Probe", "🔍 Starting", $"Fetching {url} to discover forms…");
            int fieldsTested = await XssHelper.ProbeFormsAsync(_http, url, AddRow, AddSep, ct);
            AddRow("Form XSS Probe", "✅ Complete", $"Tested {fieldsTested} field(s).");
        }
